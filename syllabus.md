# Trinity College

## *Department of Computer Science*

## CPSC 403: Computer Science Seminar, Fall 2026

*Current revision: September 7, 2026*

**Meetings:** Tuesdays. Section 01, 1:30 PM – 2:45 PM. Section 02, 2:55 PM – 4:10 PM.\
**Room:** MECC 260\
**Prerequisite:** Senior standing in Computer Science. Taken concurrently with CPSC 498 (Senior Project Part 1).\
**Credit:** Half course credit. Continues as CPSC 404 in the spring.

The two sections are identical. You are registered in one, but you may attend either section in any given week.

---

## Contact Info

**Instructor:** [Ken Kousen](https://internet3.trincoll.edu/FacProfiles/Default.aspx?fid=1000576)\
**Email:** [kkousen@trincoll.edu](mailto:kkousen@trincoll.edu)\
**Office hours:** Tuesdays, 10:00 AM – 12:00 PM, MECC 175, or by appointment

---

## Course Description

The catalog says that students engaged in research or independent study meet with faculty for oral presentations and critical discussion of papers, research plans, and progress, and that seniors using this course for the senior exercise present the results of a research or design project to the seminar.

In practice, this course is about one skill: presenting technical work so that an audience understands it, cares about it, and acts on it. The real deliverable is at the end of the spring semester, when every senior project team gives a ten-minute talk at the department's student conference. A representative of Travelers attends, and prizes go to the best talks in April. This fall builds the skills you will need to be ready for that.

We work through Nancy Duarte's *Resonate* roughly a chapter a week. Duarte's examples come from business and public life; our job is to translate them to technical talks about code, data, and systems. Alongside the reading, you give two talks, write three short pieces, and finish the semester with a preliminary pitch of your senior project to the faculty.

---

## Course Objectives

By the end of the semester you will be able to:

- Plan a talk around what the audience should believe or do afterward, not around what you know
- Structure a technical presentation so that it moves between what is and what could be
- Design slides that support the talk instead of replacing it
- Deliver a ten-minute talk inside the time limit, with a clear opening and a memorable close
- Critique a talk, yours or someone else's, using a shared vocabulary
- Pitch a senior project so that faculty believe it is worth doing

---

## Required Text

**Resonate: Present Visual Stories that Transform Audiences** by Nancy Duarte (Wiley, 2010, ISBN 978-0-470-63201-7).

A free online edition is available with registration at [resonatebook.com](https://www.resonatebook.com/). Print and Kindle editions are also fine. Read one chapter a week as listed in the schedule. The chapters are short, and each one comes up in class the week it is assigned.

---

## How the Course Works

### Two talks

**Presentation 1, the "anything" talk (Weeks 3–6).** Ten minutes, alone or with one partner, on any topic you care about: a hobby, a sport, a trip, a side project, an opinion. Senior project teams have not formed yet, so this talk deliberately separates presentation skill from content difficulty. It also lets the rest of us learn who you are.

**Presentation 2, the project pitch (Weeks 8–11).** Ten minutes, given by your senior project team. A first pass at convincing the room that your project is worth doing. Same shape as the preliminary faculty pitch at the end of the semester, lower stakes.

Sign up for a slot on the shared sign-up sheet linked from Moodle. Three slots per section per week. Upload your slides to the Moodle assignment for that round before class on the day you present.

### Three short pieces of writing

All writing in this course is **pass/redo**. There is no numeric grade. If a piece is thin, I send it back once with a note and you have a few days to resubmit.

1. **Talk analysis (due September 22).** Pick a recorded talk of an hour or less, given to a real audience, and write a one-to-two page post-mortem on it in Duarte's vocabulary. This puts the vocabulary in your hands before you present.
2. **Post-mortem 1 (due one week after your Presentation 1).** One page: what you intended, what landed, what did not, what you will change.
3. **Post-mortem 2 (due one week after your Presentation 2).** Same four parts, plus a comparison against what you said you would change after Presentation 1 and against your September talk analysis.

Each assignment has a sheet in the course repository with the full expectations.

### The preliminary faculty pitch

During finals week, each senior project team gives its ten-minute pitch to the faculty. The date will be announced once it is scheduled. The goal is the same as Presentation 2 with a different audience: convince the faculty the project is worth doing. The spring semester ends with the real thing at the student conference.

### Peer feedback and attendance

After every talk, the room asks questions and gives feedback out loud. That feedback is the main way this course works, and it only works if people are in the room. If you are absent on a day a classmate presents, you have taken something from them. Attendance is an obligation to your classmates, not to me.

---

## Senior Project (CPSC 498)

Every senior registers for CPSC 498 this fall and CPSC 499 in the spring, in addition to this seminar. This course does not run the projects; it prepares you to present them.

To register:

1. Talk to a faculty advisor and get their agreement. You need only enough of an idea for the advisor to say "yes, let's get started." Projects can and do change as they develop.
2. Submit the [special registration form](https://forms.trincoll.edu/special-registration/) on the Registrar's site.

Faculty available to advise projects this year: Profs. Syta (security-related projects), Chakraborttii, Johnson, Miyazaki, Specialetti, and Kousen. Prof. Yoon is on leave this fall. I will advise a set of projects with government and industry partners organized through the Elting Center for Innovation and Entrepreneurship and the college's AI Center; Danny Briere, who directs the Elting Center, will describe them on the first day.

Teams of one to three are all fine. With about twenty seniors across both sections, the aim is twelve or fewer project teams.

---

## Grading

| Component | Weight | Description |
|---|---|---|
| **Attendance and Participation** | 20% | Being in the room, asking questions, giving usable feedback after talks. |
| **Presentations** | 40% | Presentation 1, the "anything" talk (15%). Presentation 2, the project pitch (25%). |
| **Writing** | 20% | Talk analysis, Post-mortem 1, Post-mortem 2. Each is pass/redo; the category is the fraction passed. |
| **Preliminary Faculty Pitch** | 20% | The team's ten-minute pitch to the faculty during finals week. |

### Notes on grading

**Talks are graded on the Duarte principles, not on the topic.** Did you know what you wanted the audience to walk away with? Did the talk move? Did the slides help? Did you land inside ten minutes? A talk about your grandmother's recipes that does those things beats a talk about distributed consensus that does not.

**Writing is pass/redo.** A pass is worth one point in the gradebook; three passes is full credit for the category. A piece that comes back for a redo and then passes counts as a pass.

**Post-mortems are due one week after your own talk.** The Moodle due date on each post-mortem is the outer limit for the whole round. Your real deadline is a week after you presented.

**Swapping slots is fine; skipping is not.** If you need a different day, trade with another student or team who agrees, update the sign-up sheet, and let me know. A slot that nobody fills is only excused for a genuine emergency. If your partner or teammate is absent, the rest of the team presents.

Grade weights are subject to adjustment as the semester develops to ensure fair assignment of course grades.

### Absences and the absence memo

This course meets once a week, so one absence is a large slice of the semester. Classes are recorded on Zoom and the recording link is posted, but a recording is not the same as being in the room.

**If you miss a class, submit a one-page memo on Moodle before the next class.** Summarize the key points covered and the action items, then add one line that is yours alone: the most important takeaway and why, or a question you would have asked. If the class you missed was a presentation day, you may instead write one page of feedback to the students who presented, based on the recording.

You are welcome to use AI to draft the summary from the recording. The personal line is the part that has to be you.

**The memo is what turns an unexcused absence into an excused one.** Memos are pass/redo like the other writing. The arithmetic:

- Attendance and Participation starts at 100 points.
- An absence with a passing memo costs nothing.
- An absence with no memo, or a memo that never passes, costs 15 points. At the 20% weight, that is 3 points off the course grade per absence.
- More than four unexcused absences, a third of the meetings, zeroes the component, and we will need to talk about whether to continue in the course.
- The rest of the component is participation: asking questions and giving usable feedback after talks.

Missing the day of your own talk is a separate matter. You may trade slots with a willing classmate, but an unfilled slot without a genuine emergency costs the presentation grade, not participation.

---

## Weekly Schedule

Chapters refer to *Resonate*. Read the chapter before the class where it is listed.

### Week 1 — Tuesday, September 8
- Introductions; how the course works; the spring conference as the goal
- Chapter 1, *Why Resonate?*
- Danny Briere on Elting Center and AI Center senior projects
- CPSC 498 registration: talk to an advisor first
- Assigned: the "anything" talk (sign-up sheet opens), talk analysis

### Week 2 — Tuesday, September 15
- Chapter 2, *Lessons from Myths and Movies*; Chapter 3, *Get to Know the Hero*
- The audience as hero; what is versus what could be

### Week 3 — Tuesday, September 22
- **Due before class: Talk analysis**
- Chapter 4, *Define the Journey*
- Round 1 talks begin

### Week 4 — Tuesday, September 29
- Chapter 5, *Create Meaningful Content*
- Round 1 talks

### Week 5 — Tuesday, October 6
- Chapter 6, *Structure Reveals Insights*
- Round 1 talks

### Tuesday, October 13 — No class (Trinity Days)

### Week 6 — Tuesday, October 20
- Chapter 7, *Deliver Something They'll Always Remember*
- Round 1 talks conclude

### Week 7 — Tuesday, October 27
- **Due: Post-mortem 1** (outer limit; one week after your own talk)
- Chapter 8, *There's Always Room to Improve*
- Project teams and advisors should be settling; CPSC 498 forms in

### Week 8 — Tuesday, November 3
- Chapter 9, *Change Your World*
- Round 2 project pitches begin

### Week 9 — Tuesday, November 10
- Workshop: slides that support a technical talk
- Round 2 project pitches

### Week 10 — Tuesday, November 17
- Workshop: delivery, timing, and questions
- Round 2 project pitches

### Week 11 — Tuesday, November 24
- Coda, *Inspiration Is Everywhere*
- Round 2 project pitches conclude

### Week 12 — Tuesday, December 1
- **Due: Post-mortem 2** (outer limit; one week after your own talk)
- Preparing the faculty pitch: what changes when the audience is faculty

### Week 13 — Tuesday, December 8 *(last meeting)*
- Rehearsal for the preliminary faculty pitch

### Finals Week (December 17–23)
**Preliminary faculty pitch**, ten minutes per team, date to be announced

---

## Policy on AI Tools

This is a course about presenting. AI tools are welcome for the parts of the work where they help you think and prepare: brainstorming a topic, outlining a talk, drafting and cleaning up slides, rehearsing against a simulated audience, summarizing a recording for an absence memo. Three things stay yours:

- **The talk.** You stand up and deliver it. What you say has to be something you understand and can answer questions about.
- **The post-mortems.** These are about what happened when you presented and what you will change. They are only useful if they are honest and specific to your experience.
- **The personal line in every absence memo.**

When AI substantially shaped something you submit, say so in a sentence. That is not a penalty; it is the same disclosure you would make about any source.

This policy is the course-specific statement that Trinity asks every instructor to provide. It sits inside the college's [AI and Academic Integrity guidelines](https://www.trincoll.edu/academic-ai/students/academic-integrity/) on the [Academic AI site](https://www.trincoll.edu/academic-ai/), which every student should read. The college provides BoodleBox as a secure AI environment, and you are welcome to use it for any of the work above; other tools are fine as well, as long as you do not put other people's private information into them.

---

## Class Meetings in Unusual Circumstances

Due to bad weather or other unusual circumstances, there may be occasions when the class needs to meet virtually. In such a case, you will receive an email at least 30 minutes before class indicating that it will be virtual, using your section's regular Zoom link. Talks scheduled for that day happen on Zoom with the same time limit.

---

## Academic Dishonesty

Cases of academic dishonesty will be handled in accordance with the rules of Trinity College. Incidents of academic dishonesty void the grading policy and, in such cases, the final grade assigned for the term is at the discretion of the instructor.

---

## Policy on Class Conduct

You are expected to be polite and respectful toward the instructor and other students at all times. For details, see the Community Standards document at [https://www.trincoll.edu/dean-of-students/community-standards/](https://www.trincoll.edu/dean-of-students/community-standards/).

Public speaking is hard for many people. Laptops closed and phones away during classmates' talks. Feedback is specific and kind. No recording of a classmate's talk without their permission.

Signs of non-engagement, such as using cell phones or laptops for non-class purposes, will significantly impact your class participation grade, as engagement is the cornerstone of participation.

---

## Accommodations

Trinity College is committed to creating an inclusive and accessible learning environment consistent with the Americans with Disabilities Act. Students with disabilities who may need accommodations in order to fully participate in this class are urged to contact the Student Accessibility Resource Center (SARC) as soon as possible to explore what arrangements need to be made to assure access.

**If you have approval for academic accommodations, please notify me by the end of week two of classes.** This includes accommodations related to public speaking. For students with accommodations approved after the start of the semester, a minimum of 10 days' notice is required. Please meet with me privately to discuss implementation. SARC can be reached at [SARC@trincoll.edu](mailto:SARC@trincoll.edu).

---

## Academic Honesty Pledge

All students must sign the following declaration at the first meeting of every course:

> In accordance with Article II of the Trinity College Student Integrity Contract, I hereby pledge that the papers, exams, and other academic exercises I submit for this course will represent my own work; that I will properly acknowledge and attribute any and all information and ideas that I have used from other sources; and that no collaboration unauthorized by the instructor of the course will occur in the course of its completion.
>
> I have read and understood the class policy statement, especially as it pertains to grading, academic dishonesty, classroom policies and required meetings outside of class time.

Name: _______________________   Phone (optional): _______________________

Dropping this policy statement in the designated drop box will act as a signature for acknowledging that you have read and understood this statement. Please type your phone number into the comments on Moodle when dropping the document (if you are comfortable providing your phone number).

---

*The information in this syllabus may be updated during the course. Check Moodle regularly for the most current version.*
